...
/* This function will be called on every timer interrupt. Notice the void* pinter - task functions 
     can be used for more than one purpose, each time getting a different parameter. */
...
ssize_t procfile_read(char * buffer, char ** buffer_location, off_t offset, int buffer_length, int *eof, void *data)
{
     ...
     /* Its static so it will still be in memory when we leave this function */          
     static char my_buffer[80];

     static int count = 1;

     /* We give all of our information in one go, so if the anybody asks us if we have more 
...
void __exit    cleanup_module()
{
   ...
   die = 1;    /* Keep intrp routine from queueing itself */
   cancel_delayed_work(&Task);     /* no "new ones" */
   flush_workqueue(my_workqueue);  /* wait till all 'old ones' finiished */
   destroy_workqueue(my_workqueue);     

   /* Sleep until intrpt_routine is called one last time. 
          This is necessary, because otherwise we'll deallocate the memory holding intrpt_routine 
          and Task while work_timer still references them. Notice that here we don't allow signals to 
          interrupt us.
