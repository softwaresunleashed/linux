/*
     ...
     The declarations here have to be in a header file, because they need to be known both to the 
     kernel module (in chardev.c) and the process calling ioctl (ioctl.c)
*/
...
/* This IOCTL is used for output,  to get the message of the device driver.
     However, we still need the buffer to place the message in to be input, as it is allocated by the 
     process. 
*/
...
/*
     The IOCTL is used for both input and output. It receives from the user a number, n, 
     and returns Message[n].
*/
