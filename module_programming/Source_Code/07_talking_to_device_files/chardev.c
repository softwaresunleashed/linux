...
/* This function is called whenever a process which has already opened the device file 
                             ...
                             loff_t   * offset)
{
     ...
     /* If we're at the end of the message, return 0 (which signifies EOF) */
     if(*Message_Ptr == 0)
          return 0;
     ...
     /* Read functions are supposed to return the number of bytes actually inserted
               into the buffer */
     return bytes_read;
...
We get two extra parameters (additional to the inode and file structures, which all device 
functions get) : the number of the ioctl called and the parameter given to the ioctl function. 

If the ioctl is write or read/write (meaning output is returned to the calling process), the ioctl call 
     ...
     switch(ioctl_num)
     {
          case IOCTL_SET_MSG:
               /* Receive a pointer to a message (in user space) and set that to be the 
                              devices message. Get the parameter given to ioctl by the process. */
          ...
          case IOCTL_GET_MSG:
               /* Give the current message to the calling process the parameter we got is a pointer, fill it. */
...
/* This structure will hold the functions to be called when a process does something to the device
we created. Since a pointer to this structure is kept in the devices table, it can't be local to
...
int init_module()
{
     ...
     if(ret_val < 0){
          printk(KERN_INFO "Sorry, registering the character device failed with %d\n", ret_val);
          return ret_val;
     }
